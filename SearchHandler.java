...
public class SearchHandler implements Handler{
	...
	public void handleIt(String[] parameters) {
		...
		resorts = model.getResortsFromZipCode(parameters[0], Integer.parseInt(parameters[1]));
		
		//if there were no results try the resort name
		if (resorts.size() == 0) {
			resorts = model.getResortsFromName(parameters[0], Integer.parseInt(parameters[1]));
		}
		
		//if there were no results try the city state string
		if (resorts == null) {
			cityState = this.parseCityState(parameters[0]);
			if (cityState != null) {
				resorts = model.getResortsFromCityState(cityState[0], cityState[1], Integer.parseInt(parameters[1]));
			}
		}		
